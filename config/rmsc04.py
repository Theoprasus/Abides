...
agents.extend([WbrAgent(id=j,
                          name="WbrAgent {}".format(j),
                          type="WbrAgent",
                          symbol=symbol,
                          starting_cash=starting_cash,
                          wake_up_freq='40s',
               ...
               for j in range(agent_count, agent_count + num_wbr)])
agent_count += num_wbr
agent_types.extend(['WbrAgent'])
...
agent_count += num_bh
agent_types.extend(['BHAgent'])

# 5e) DynamicCppi Agent
num_DynCppi = 20
agents.extend([DynamicCppiAgent(id=j,
                          name="Dynamic cppi Agent {}".format(j),
                          type="DynamicCppiAgent",
                          symbol=symbol,
                          starting_cash=starting_cash,
                          sigma_n=sigma_n,
                          r_bar=r_bar,
                          ...
                          log_orders=log_orders,
                          random_state=np.random.RandomState(seed=np.random.randint(low=0, high=2 ** 32, dtype='uint64')))
               for j in range(agent_count, agent_count + num_DynCppi)])
agent_count += num_DynCppi
agent_types.extend(['DynamicCppiAgent'])
